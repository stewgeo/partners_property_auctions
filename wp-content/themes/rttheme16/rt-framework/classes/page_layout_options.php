...
class RTThemePageLayoutOptions extends RTThemeAdmin{
	...
	function rt_save_page_templates(){
		...
		foreach($savedTemplates as $key=>$value){
			...
			if(!$jump){
				...
				$templates->templates[$template]->contents[$box]->box_border         =  $value["box_border"];
				$templates->templates[$template]->contents[$box]->pagination         =  $value["pagination"];
				...
				$templates->templates[$template]->contents[$box]->categories         =  $value["categories"]; 
				}
				...
				$templates->templates[$template]->contents[$box]->layout             =  $value[0];
				$templates->templates[$template]->contents[$box]->sidebar_id         =  $value[1];
				$templates->templates[$template]->contents[$box]->widget_box_width   =  $value[2];
				$templates->templates[$template]->contents[$box]->widget_border      =  $value[3]; 
				}
				...
				$templates->templates[$template]->contents[$box]->content_id         =  $value["content_id"];					
				$templates->templates[$template]->contents[$box]->layout             =  $value["layout"]; 
				$templates->templates[$template]->contents[$box]->item_width         =  $value["item_width"];
				$templates->templates[$template]->contents[$box]->box_border         =  $value["box_border"];
				$templates->templates[$template]->contents[$box]->list_orderby       =  $value["list_orderby"];
				...
				$templates->templates[$template]->contents[$box]->button_text        =  $value["button_text"];
					...
					wpml_register_string( THEMESLUG , 'Text for Banner '.$templates->templates[$template]->templateID, $value["text"] );
					wpml_register_string( THEMESLUG , 'Button Text for Banner '.$templates->templates[$template]->templateID, $value["button_text"] );
					wpml_register_string( THEMESLUG , 'Button Link for Banner '.$templates->templates[$template]->templateID, $value["button_link"] );
					
				$templates->templates[$template]->contents[$box]->button_color       =  $value["button_color"];
				$templates->templates[$template]->contents[$box]->button_link        =  $value["button_link"];
				$templates->templates[$template]->contents[$box]->button_size        =  $value["button_size"]; 
				}
				...
				$templates->templates[$template]->contents[$box]->description        =  $value["description"];
					...
					//wpml register string
					wpml_register_string( THEMESLUG , 'Text for Contact Form '.$templates->templates[$template]->templateID, $value["title"] );
					wpml_register_string( THEMESLUG , 'Description for Contact Form '.$templates->templates[$template]->templateID, $value["description"] );
				}
				...
				$templates->templates[$template]->contents[$box]->box_border         =  $value["box_border"];
					...
					//wpml register string
					wpml_register_string( THEMESLUG , 'Title for Contact Info '.$templates->templates[$template]->templateID, $value["contact_title"] );
					wpml_register_string( THEMESLUG , 'Text for Contact Info '.$templates->templates[$template]->templateID, $value["contact_text"] ); 
					wpml_register_string( THEMESLUG , 'Address for Contact Info '.$templates->templates[$template]->templateID, $value["address"] );
					wpml_register_string( THEMESLUG , 'Phone for Contact Info '.$templates->templates[$template]->templateID, $value["phone"] ); 
					wpml_register_string( THEMESLUG , 'Email for Contact Info '.$templates->templates[$template]->templateID, $value["email"] );
					wpml_register_string( THEMESLUG , 'Support Email for Contact Info '.$templates->templates[$template]->templateID, $value["support_email"] ); 
					wpml_register_string( THEMESLUG , 'Fax for Contact Info '.$templates->templates[$template]->templateID, $value["fax"] ); 				
				}
	...
	function rt_create_default_templates(){
		...
		$defaultTemplates = new stdClass;
				...
				$defaultTemplates->templates[$templateID]->contents[0]->slider_script      = "accordion";
				...
				$defaultTemplates->templates[$templateID]->contents[0]->slider_effect      = "scrollUp";
				...
				$defaultTemplates->templates[$templateID]->contents[1]->text               = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor <a href="#" title="">do eius mod tempor</a> labore et dolore magna aliqua. Ut enim ad minim veniam.';
				$defaultTemplates->templates[$templateID]->contents[1]->button_text        = '';
				...
				$defaultTemplates->templates[$templateID]->contents[2]->box_border         = "on";
				...
				$defaultTemplates->templates[$templateID]->contents[3]->widget_border      = "on";
				...
				$defaultTemplates->templates[$templateID]->contents[2]->box_border         = "";
	...
	function rt_generate_banner_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$button_color  = $isNewBox ? ''				:	$options['button_color'];
			$button_size   = $isNewBox ? ''				:	$options['button_size']	;  
			...
			$options = array (
					...
					array(
							   "name" => __("Select Button Size",'rt_theme_admin'),
							   "desc" => __("Please select a box size for the content.",'rt_theme_admin'),
							   "id" => $theTemplateID.'_'.$theGroupID."_banner_box[button_size]",
							   "options" =>	array(
												"small"		=>	__("Small Button",'rt_theme_admin'),
												"medium"		=>	__("Medium Button",'rt_theme_admin'),
												"big"		=>	__("Big Button",'rt_theme_admin'),					
											),
					...
					array(
							   "name" => __("Select Button Color",'rt_theme_admin'),
							   "id" => $theTemplateID.'_'.$theGroupID."_banner_box[button_color]",
							   "options" =>	array(
												"light"		=>	__("Light",'rt_theme_admin'),
												"dark"		=>	__("Dark",'rt_theme_admin'),
												"orange"		=>	__("Orange",'rt_theme_admin'),
												"green"		=>	__("Green",'rt_theme_admin'),
												"blue"		=>	__("Blue",'rt_theme_admin'),
												"red"		=>	__("Red",'rt_theme_admin'),																	
											),
	...
	function rt_generate_slider_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							"desc" 		=> __("This module shows a slider.",'rt_theme_admin'),	 
							"hr" 		=> "true",
							"type" 		=> "info_text_only"), 							
					...
					array(
							...
							"desc" 		=> __("Please select contetst which you want to display in this box. If you don't select one, all contents will be displayed.",'rt_theme_admin'),
					...
					array(
							...
							"hr" 		=> "true",
							"class"		=> $randomClass, 
							"value"		=> $isNewBox ? "on" : $options["slider_buttons"], 
							"type" 		=> "checkbox"),
	...
	function rt_generate_all_content_boxes($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$box_border	= $isNewBox ? ''			:	$options['box_border']	;			
			...
			$options = array (
					...
					array(
							"name" 		=> __("Box Border",'rt_theme_admin'), 
							"desc" 		=> __('adds a border each contents if the "Full Width" selected as the column with, otherwise adds a border for the column only.','rt_theme_admin'),
							"id" 		=> $theTemplateID.'_'.$theGroupID."_all_content_boxes[box_border]",
							"type" 		=> "checkbox",
							"class"		=> $randomClass,
							"value"		=> $box_border, 							
							"hr"			=> true,
	...
	function rt_generate_portfolio_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							"name" 	=> __("Box Border",'rt_theme_admin'),
							"desc" 	=> __('adds a border around all the portfolio posts if the "Full Width" is NOT selected as "Column Width".','rt_theme_admin'),
							"id" 	=> $theTemplateID.'_'.$theGroupID."_portfolio_box[box_border]",
							"type" 	=> "checkbox",
							"class"	=> $randomClass,
							"value"	=> $box_border, 							
							"hr"		=> true,
	...
	function rt_generate_product_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							"name" 	=> __("Box Border",'rt_theme_admin'),
							"desc" 		=> __('adds a border around all the product posts if the "Full Width" is NOT selected as "Column Width".','rt_theme_admin'),
							"id" 	=> $theTemplateID.'_'.$theGroupID."_product_box[box_border]",
							"type" 	=> "checkbox",
							"class"	=> $randomClass,
							"value"	=> $box_border, 							
							"hr"		=> true,
	...
	function rt_generate_sidebar_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$widget_border		= $isNewBox ? ''			:	$options['widget_border']	;
			...
			$options = array (
					...
					array(
							   "name" => __("Column Width",'rt_theme_admin'),
							   "desc" => __("Please select a column size for this module.",'rt_theme_admin'),
							   "id" => $theTemplateID.'_'.$theGroupID."_sidebar_box[]",
							   "options" =>	array(
													"one"		=>		"Full Width",
													"two"		=>		"1:2 - One Second",
													"three"		=>		"1:3 - One Third",
					...
					array(
							"name" 	=> __("Box Border",'rt_theme_admin'),
							"desc" 	=> __('adds a border each widget if the "Full Width" selected as the column with, otherwise adds a border for the column only.','rt_theme_admin'),
							"id" 	=> $theTemplateID.'_'.$theGroupID."_sidebar_box[]",
							"type" 	=> "checkbox",
							"class"	=> $randomClass,
							"value"	=> $widget_border, 							
	...
	function rt_generate_contact_form($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							"name" 	=> __("STYLING",'rt_theme_admin'), 
							"type" 	=> "heading"), 
		
					array(
							"name" 	=> __("Box Border",'rt_theme_admin'),
							"desc" 	=> __('adds a border for this module.','rt_theme_admin'),
							"id" 	=> $theTemplateID.'_'.$theGroupID."_contact_form[box_border]",
							"type" 	=> "checkbox",
							"class"	=> $randomClass,
							"value"	=> $options['box_border'], 							
							"hr"		=> true,
							"default"	=> "off",
							"std" 	=> "false"),
	...
	function rt_generate_contact_info_box($theGroupID,$theTemplateID,$options,$randomClass){
			...
			$options = array (
					...
					array(
							"name" 	=> __("Box Border",'rt_theme_admin'),
							"desc" 	=> __('adds a border for this module.','rt_theme_admin'),
							"id" 	=> $theTemplateID.'_'.$theGroupID."_contact_info_box[box_border]",
							"type" 	=> "checkbox",
							"class"	=> $randomClass,
							"value"	=> $options['box_border'], 							
							"hr"		=> true,
