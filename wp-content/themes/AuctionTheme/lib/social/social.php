...
******************************************************************************/
	 ...
	 function AuctionTheme_render_login_page_uri(){
	?>
...
*****************************************************************************************/
	...
	function AuctionTheme_display_social_login()
	{
		
		$images_url = get_bloginfo('template_url') . "/lib/social/img/";
		$AuctionTheme_enable_facebook_login = get_option('AuctionTheme_enable_facebook_login');
		$AuctionTheme_enable_twitter_login 	= get_option('AuctionTheme_enable_twitter_login');
		
		if($AuctionTheme_enable_facebook_login == "yes" or $AuctionTheme_enable_twitter_login == "yes"):
		?>
        
        	<p>
					<label><?php _e('Connect with:','AuctionTheme'); ?></label>
        
        
        <?php
		
		 if( $AuctionTheme_enable_facebook_login == "yes" ) : ?>
			...
			<?php endif; ?>
			<?php if( $AuctionTheme_enable_twitter_login == "yes" ) : ?>
		...
		<input type="hidden" name="client_id" value="<?php echo get_option( 'AuctionTheme_facebook_app_id' ); ?>" />
...
*****************************************************************************************/
	...
	function AuctionTheme_social_connect_process_login( $is_ajax = false ){
	...
	switch( $social_connect_provider ) {
		case 'facebook':
		social_connect_verify_signature2( $_REQUEST[ 'social_connect_access_token' ], $sc_provided_signature, $redirect_to );
		$fb_json = json_decode( sc_curl_get_contents2("https://graph.facebook.com/me?access_token=" . $_REQUEST[ 'social_connect_access_token' ]) );
		...
		social_connect_verify_signature2( $sc_provider_identity, $sc_provided_signature, $redirect_to );
	...
	// Get user by meta
	$user_id = social_connect_get_user_by_meta2( $sc_provider_identity_key, $sc_provider_identity );
	if ( $user_id ) {
	...
	$redirect_to = auctionTheme_my_account_link();
	$redirect_to = apply_filters('AuctionTheme_filter_social_redirect_url', $redirect_to, $social_connect_provider);
...
*****************************************************************************************/
	...
	add_action( 'login_form_social_connect', 	'AuctionTheme_social_connect_process_login');
	add_filter('login_form', 					'AuctionTheme_display_social_login');
	add_action( 'wp_footer', 					'AuctionTheme_render_login_page_uri' );
	add_filter('register_form', 					'AuctionTheme_display_social_login');
