...
class AuctionTheme_best_rated_users extends WP_Widget {
	...
	function AuctionTheme_best_rated_users() {
		...
		$this->WP_Widget( 'best-rated-users', 'AuctionTheme - Best Rated Users', $widget_ops, $control_ops );
	}
		...
		$querystr = "SELECT users.user_email email, users.user_registered dt, users.user_login username, ratings.touser uid, 
				...
				else
				{
					...
					foreach($r as $row) // = mysql_fetch_object($r))
					{
						...
						echo '<td width="20%"><a href="'.get_bloginfo('siteurl').'/user-profile/'.$row->username.'">';
						...
						Joined on: '.$dt.'
						<br/>'.AuctionTheme_get_auction_stars($row->rate).'
						</td>';
	...
	function form($instance) { ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('user_limit'); ?>"><?php _e('Users Limit'); ?>:</label>
