...
$client_id = get_option('AuctionTheme_facebook_app_id');
$secret_key = get_option('AuctionTheme_facebook_app_secret');
...
if(isset($_GET['code'])) {
  ...
  $client_id = get_option('AuctionTheme_facebook_app_id');
  $secret_key = get_option('AuctionTheme_facebook_app_secret');
  parse_str(sc_curl_get_contents2("https://graph.facebook.com/oauth/access_token?" .
    'client_id=' . $client_id . '&redirect_uri=' . urlencode(get_bloginfo('template_url') . '/lib/social/facebook/callback.php') .
    '&client_secret=' .  $secret_key .
    '&code=' . urlencode($code)));
    
  $signature = social_connect_generate_signature2($access_token); 
