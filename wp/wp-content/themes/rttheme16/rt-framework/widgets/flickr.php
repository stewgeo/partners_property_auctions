...
class Flickr_Widget extends WP_Widget {
	...
	function Flickr_Widget() {
		...
		$this-> WP_Widget('flickr', '['. THEMENAME.']   '.__('Flickr', 'rt_theme_admin'), $opts);
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<script type="text/javascript">jQuery("#'.$randomID.'").jflickrfeed({limit: '.$show_image.',qstrings: {id: \''.$flickr_username.'\' }, itemTemplate: \'<li><span class="border"><a href="{{image_b}}"><img src="{{image_s}}" alt="{{title}}" /></a></span></li>\'});</script>';		
		echo $after_widget;
	}
...
?>
		...
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo empty($title) ? __('Flickr Feeds','rt_theme') : $title; ?>" /></p>
