...
class Popular_Posts extends WP_Widget {
	...
	function Popular_Posts() {
		...
		$this-> WP_Widget('popular_posts', '['. THEMENAME.']   '.__('Popular Posts', 'rt_theme_admin'), $opts);
	}
	...
	function widget( $args, $instance ) {
		...
		$postargs			=	($all_post_formats) ?
							array('orderby'=> 'comment_count','post_type'=>'post','showposts'=>$count,'cat'=>$categories, 'nopaging' => 0, 'post_status' => 'publish', 'ignore_sticky_posts' => 1 ) :
							array('orderby'=> 'comment_count','tax_query' => array( array( 'operator' => 'NOT IN', 'taxonomy' => 'post_format', 'field' => 'slug', 'terms' => array( 'post-format-quote' , 'post-format-aside' ) ) ),'post_type'=>'post','showposts'=>$count,'cat'=>$categories, 'nopaging' => 0, 'post_status' => 'publish', 'ignore_sticky_posts' => 1 ) ;
		...
		if ($post_query->have_posts()) : while ($post_query->have_posts()) : $post_query->the_post();							
			...
			if ( has_post_thumbnail() && !$show_thumbnails) :
				...
				$f_image 		= 	@vt_resize( $f_thumb, '', $thumb_width, $thumb_height, true );			
...
?>
		...
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo empty($title) ? __('Popular Posts','rt_theme') : $title; ?>" /></p>
