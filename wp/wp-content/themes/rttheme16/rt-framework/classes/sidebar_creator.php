...
class RT_Create_Sidebars{
	...
	function rt_sidebar($sidebar_id,$sidebar_name){ 
		...
		if($sidebar_id=="sidebar-for-footer"){						
			...
			$footer_page_layout= $this->sidebar_classes[0][get_option(THEMESLUG."_footer_box_width")];

			//footer widget styles
			$footer_widget_styles= get_option(THEMESLUG."_footer_widget_styles") ? "border white" : "";
			
			register_sidebar(array(
				'id'            => $sidebar_id,
				'name'          => $sidebar_name,
				'before_widget' => '<div class="box '.$footer_page_layout.' column_class '.$footer_widget_styles.' footer widget"><div class="padding-div">',
			...
			register_sidebar(array(
				'id'            => $sidebar_id,
				'name'          => $sidebar_name,
				'before_widget' => '<div class="box border box_layout column_class widget white"><div class="featured"><div class="padding-div">',
				'description'   => $sidebar_descriptions,
				'after_widget'  => '</div></div></div>',
				'before_title'  => '<div class="title"><h5>',
				'after_title'   => '</h5><!--<div class="space margin-b10"></div>--></div>',
			));					
		}
	...
	function home_page_layout_class($params) {
		...
		if($params[0]['id'] == $sidebarID){			
			...
			// clear
			if(fmod($widget_num,$box_width) == 0  && RT_Create_Sidebars::count_sidebar_items($id)!=$widget_num ):
			...
			if(!$widget_border)  {$params[0]['before_widget'] = str_replace('border', "", $params[0]['before_widget']) ;}
			...
			$params[0]['after_widget']  = $params[0]['after_widget'] .''. $reset;
		}
	...
	function footer_layout_class($params) {
		...
		if($params[0]['id'] == "sidebar-for-footer"){
			...
			// clear
			if(empty($footer_contents_count)) echo '<div class="clear"></div><div class="space margin-b10"></div>';			 
			...
			if(fmod($widget_num,$box_width) == 0 || $box_width==1 || $this->count_sidebar_items($id)==$widget_num):
			    $reset = (get_option(THEMESLUG."_footer_widget_styles")) ? '<div class="space margin-b30"></div>' : '<div class="space margin-b10"></div>';
			endif;
	...
	function rt_get_sidebar(){
		...
		if(is_home()) dynamic_sidebar('sidebar-for-home-page');
		...
		// Blog All
		if( is_blog_page() ) dynamic_sidebar('sidebar-for-blog');
